...
// 语义(重要,别误读成"锁一次就恢复"):
// 失败计数是**累积**的,只在登录成功、或 1 小时无任何失败后清零。因此计数达到某一档之后,
// **每一次后续失败都会按该档重新计时**——第 3 次失败锁 30 秒;等窗口过去再失败一次,仍是锁 30 秒,
// 直到某次成功、或计数继续涨到 5 / 8 进入更长的档。
//
// 为什么不做成"锁过期即清零":那样攻击者每轮都能拿回 3 次免费尝试,计数永远涨不到 5,
// 后面的 5 分钟 / 30 分钟档就形同虚设。累积计数是让爆破速率被真正压下去的关键。
//
// 对本人误伤有限:单用户工具下,忘了口令可以直接重生成 .env 里的 AUTH_PASSWORD_HASH,不存在被永久锁死。
